...
export const EnergyTab: React.FC<EnergyTabProps> = ({ summary, history }) => {
  ...
  // Floor breakdown bar data
  const floorData = summary.building_utilization.floor_utilizations.map((fl) => ({
    name: fl.floor_name.split(' - ')[0],
    power: fl.total_power_kw,
    occupied: fl.occupied_count,
    total: fl.total_count,
  }));

  const deviation = summary.current_energy_kw - summary.expected_energy_kw;
  const deviationPct = summary.expected_energy_kw > 0 ? (deviation / summary.expected_energy_kw) * 100 : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200/80 rounded-xl p-4 shadow-xs">
          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Actual Power Draw</span>
          <div className="mt-2 flex items-baseline space-x-2">
            <span className="text-2xl font-bold text-slate-900">{summary.current_energy_kw.toFixed(1)}</span>
            <span className="text-xs font-semibold text-slate-500">kW</span>
          </div>
          <div className="mt-1 text-xs text-slate-500">Sum of 24 zone sub-meters</div>
        </div>

        <div className="bg-white border border-slate-200/80 rounded-xl p-4 shadow-xs">
          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Expected Model Baseline</span>
          <div className="mt-2 flex items-baseline space-x-2">
            <span className="text-2xl font-bold text-slate-700">{summary.expected_energy_kw.toFixed(1)}</span>
            <span className="text-xs font-semibold text-slate-500">kW</span>
          </div>
