...
export const WastageTab: React.FC<WastageTabProps> = ({
  ...
  const wastages = summary.wastages;
  const totalWastedKw = wastages.reduce((acc, w) => acc + w.wasted_power_kw, 0);
  const totalWastedHourlyKwh = totalWastedKw;
  const monthlyCostWaste = totalWastedHourlyKwh * 24 * 30 * summary.tariff_rate_usd_per_kwh;

  const hvacWastages = wastages.filter((w) => w.type === 'hvac_wastage');
  const lightingWastages = wastages.filter((w) => w.type === 'lighting_wastage');
  const equipmentWastages = wastages.filter((w) => w.type === 'equipment_abnormal_load');
  const utilizationWastages = wastages.filter((w) => w.type === 'low_utilization_high_energy');
